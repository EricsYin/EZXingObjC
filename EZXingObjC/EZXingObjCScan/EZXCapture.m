...
@interface EZXCapture ()<CALayerDelegate>
...
- (void)setDelegate:(id<EZXCaptureDelegate>)delegate {
    _delegate = delegate;
    ...
    @autoreleasepool {
        if (!self.cameraIsReady) {
            ...
            if ([self.delegate respondsToSelector:@selector(captureCameraIsReady:)]) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.delegate ECaptureCameraIsReady:self];
                });
            }
        }
        ...
        if (tempImage) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(zxingLuminanceSourceImage:)]) {
                    [self.delegate zxingLuminanceSourceImage:tempImage];
                }
        ...
        if (self.binaryLayer || self.delegate)
        {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(captureResult:result:scanImage:)]){
...
// 重复执行的定时器
- (void)gcdTimer1Image:(UIImage *)image{
...
#pragma mark - Public define method
